...
private func authorizationOtherSessionCodeText(phoneNumber: String, strings: PresentationStrings) -> String {
    ...
    switch languageCode {
    case "ru":
        return "Введите код, отправленный в приложение на другом вашем устройстве для **\(phoneNumber)**."
    case "uk":
        return "Введіть код, надісланий у застосунок на іншому вашому пристрої для **\(phoneNumber)**."
    default:
        return "Enter the code sent to the app on your other device for **\(phoneNumber)**."
    }
}
...
public func authorizationCurrentOptionText(_ type: SentAuthorizationCodeType, phoneNumber: String, email: String?, strings: PresentationStrings, primaryColor: UIColor, accentColor: UIColor) -> NSAttributedString {
    ...
    case .otherSession:
        return parseMarkdownIntoAttributedString(authorizationOtherSessionCodeText(phoneNumber: phoneNumber, strings: strings), attributes: attributes, textAlignment: .center)
    case .missedCall:
        let body = MarkdownAttributeSet(font: Font.regular(fontSize), textColor: primaryColor)
        let bold = MarkdownAttributeSet(font: Font.semibold(fontSize), textColor: primaryColor)

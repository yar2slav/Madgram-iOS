...
public final class ChatListContainerNode: ASDisplayNode, ASGestureRecognizerDelegate {
        ...
        openArchiveSettings: @escaping () -> Void)
    {
        ...
        let panRecognizer = InteractiveTransitionGestureRecognizer(target: self, action: #selector(self.panGesture(_:)), allowedDirections: { [weak self] _ in
            guard let self, self.availableFilters.count > 1 || (self.controller?.isStoryPostingAvailable == true && !(self.context.sharedContext.callManager?.hasActiveCall ?? false) && !InterfaceTuningSettingsStore.shared.current.disableStoryCameraSwipe) else {
                return []
            }
    ...
    @objc private func panGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            if let (layout, navigationBarHeight, visualNavigationHeight, originalNavigationHeight: originalNavigationHeight, cleanNavigationBarHeight, insets, isReorderingFilters, isEditing, inlineNavigationLocation, inlineNavigationTransitionFraction, storiesInset) = self.validLayout, let selectedIndex = self.availableFilters.firstIndex(where: { $0.id == self.selectedId }) {
                ...
                if case .compact = layout.metrics.widthClass, self.controller?.isStoryPostingAvailable == true && !(self.context.sharedContext.callManager?.hasActiveCall ?? false) && !InterfaceTuningSettingsStore.shared.current.disableStoryCameraSwipe {
                    if hasLiveStream {
